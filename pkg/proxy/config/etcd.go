...
func (impl ConfigSourceEtcd) ProcessChange(response *etcd.Response) {
	log.Printf("Processing a change in service configuration... %v", *response)
	...
	if strings.Contains(response.Node.Key, "/endpoints/") {
		...
		if err != nil {
			log.Printf("Failed to parse %v Port: %s", response, err)
			return
		}
...
func (impl ConfigSourceEtcd) ProcessEndpointResponse(response *etcd.Response) {
	log.Printf("Processing a change in endpoint configuration... %v", *response)
	var endpoints api.Endpoints
